...
export function MobileMenu() {
  ...
  return (
    <>
      {/* 모바일: 오른쪽 중앙 고정 햄버거 버튼 */}
      <button
        onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
        className="fixed right-4 top-1/2 z-50 -translate-y-1/2 flex h-12 w-12 items-center justify-center rounded-full bg-[var(--color-primary)] text-white shadow-lg transition hover:opacity-90 sm:hidden"
        aria-label="메뉴 열기"
      >
        <svg
          className="h-6 w-6"
